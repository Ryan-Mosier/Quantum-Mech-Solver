    ...
    for (auto& c : line)
    {
        if (TokenType t = trySingleCharTokenize(c); t != TokenType::Null)
        {
            ...
            if (!tokens.empty())
            {
                ...
                tokens.back().adj.second = &newToken;
                newToken.adj.first = &tokens.back();
            }
...
void Parser::parse()
{
    for (const auto& t : srcTokens)
    {
        auto expr = parseLine(t);
    }
}

Expression Parser::parseLine(std::vector<Token> lineTokens)
{
